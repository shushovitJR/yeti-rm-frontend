...
function Reports() {
  ...
  const fetchMonthlyData = async () => {
    try {
      const [repairsData, requestsData] = await Promise.all([
        reportAPI.getMonthlyRepairs(),
        reportAPI.getMonthlyRequests(),
      ]);

      const mergedData = repairsData.map((repairItem) => {
        const requestItem = requestsData.find(
          (req) => req.month === repairItem.month
        );

        return {
          month: repairItem.month,
          repairs: repairItem.repairs,
          completed: repairItem.completed + requestItem.completed,
      ...
      const errMsg =
        error.data?.message || error.message || "Failed to fetch monthly data";
